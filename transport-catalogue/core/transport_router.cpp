...
std::optional<BestRouteInfo> TransportRouter::MakeRoute(const Stop *from, const Stop *to)
{
    ...
    if (auto iter = comming_stop_.find(from); iter != comming_stop_.end())
        from_index = iter->second;
    else
        return {};

    if (auto iter = comming_stop_.find(to); iter != comming_stop_.end())
        to_index = iter->second;
    else
        return {};

    if (auto route = router_ptr_->BuildRoute(from_index, to_index))
...
void TransportRouter::MakeRouteEdges()
{
    ...
    for (const auto &route : routes)
    {
        ...
        {
            auto midle_stop_iter = route->stops_list.begin() + route->end_stop_number_;
                                      ...
                                      const Route *route)
{
    ...
    VSIter curent_stop;
    size_t route_size = std::distance(first_stop, last_stop);
    edge_stops_.reserve(route_size * route_size / 2);
    while (start_stop != last_stop)
    {
        curent_stop = start_stop + 1;
        double weight = 0;
        int span_count = 1;
...
BestRouteInfo TransportRouter::MakeItems(std::vector<size_t> &edges) const
{
    ...
    for (auto &edge : edges)
    {
        RouteItem item = edge_stops_.find(edge)->second;
        weight += item.time;
