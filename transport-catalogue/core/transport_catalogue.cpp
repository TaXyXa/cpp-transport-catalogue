...
Stop *TransportCatalogue::GetStop(const std::string &name) const
{
    auto it = stops_reference_.find(name);
    if (it == stops_reference_.end())
    {
...
uint32_t TransportCatalogue::GetDistance(Stop *curent_stop, Stop *second_stop) const
{
    ...
    return static_cast<uint32_t>(ComputeDistance(curent_stop->coordinates, second_stop->coordinates));
}
