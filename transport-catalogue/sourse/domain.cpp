...
{
    size_t hash = std::hash<std::string>{}(stop_name.first->name) * std::hash<std::string>{}(stop_name.first->name) + std::hash<std::string>{}(stop_name.second->name);
    return hash;
}
...
{
    return static_cast<size_t>(coordinate.lat * 42 + coordinate.lng * 42 * 42);
}