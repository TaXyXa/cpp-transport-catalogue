...
Stop *RequestHandler::AddStop(const std::string &name, const Coordinates &coordinates)
{
...
void RequestHandler::AddDistance(Stop *curent_stop, Stop *second_stop, uint32_t distance)
{
    catalogue_.AddDistance(curent_stop, second_stop, distance);
}

void RequestHandler::SetSettings(const renderer::Setting &setting)
{
    map_renderer_.SetSettings(setting);
}

std::optional<BestRouteInfo> RequestHandler::MakeRoute(const std::string &from, const std::string &to)
{
...
void RequestHandler::SetRouteSettings(int wait_time, int velocity)
{
